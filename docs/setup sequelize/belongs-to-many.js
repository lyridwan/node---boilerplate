...
class BelongsToMany extends Association {
  ...
  _injectAttributes() {
    ...
    const { sourceKeyField } = this;
    const targetKey = this.target.rawAttributes[this.targetKey];
    const targetKeyType = targetKey.type;
    const { targetKeyField } = this;
    ...
    this.through.model.rawAttributes[this.foreignKey] = Object.assign(
      this.through.model.rawAttributes[this.foreignKey],
      sourceAttribute,
    );
    this.through.model.rawAttributes[this.otherKey] = Object.assign(
      this.through.model.rawAttributes[this.otherKey],
      targetAttribute,
    );
  ...
  has(sourceInstance, instances, options) {
    ...
    return this.get(sourceInstance, options).then(
      associatedObjects =>
  ...
  set(sourceInstance, newAssociatedObjects, options) {
    ...
    const { sourceKey } = this;
    const { targetKey } = this;
    const { identifier } = this;
    const { foreignIdentifier } = this;
    let where = {};
    ...
    return this.through.model
      .findAll(_.defaults({ where, raw: true }, options))
  ...
  add(sourceInstance, newInstances, options) {
    ...
    const { targetKey } = association;
    const { identifier } = association;
    const { foreignIdentifier } = association;
    const defaultAttributes = options.through || {};
  ...
  create(sourceInstance, values, options) {
    ...
    return association.target
      ...
      .then(newAssociatedObject =>
        sourceInstance[association.accessors.add](newAssociatedObject, _.omit(options, ['fields'])).return(
          newAssociatedObject,
        ),
      );
  }
